...
type Config struct {
	...
	// It is required.
	Now func() time.Time
	// Seed seeds the pseudo-random address generator. Per RFC3927 section 2.1 it
	// SHOULD be derived from a persistent per-host value such as the MAC address so
	// that different hosts pick different sequences and a host tends to reuse the same
	// address across reboots. If zero, a seed is derived from HardwareAddr.
	Seed uint64
	// FirstCandidate, if non-zero and within 169.254.1.0-169.254.254.255, is tried
	// before any random address. Use it to retry a previously recorded address.
	FirstCandidate [4]byte
}
...
func (h *Handler) Reset(cfg Config) error {
	if cfg.Now == nil || internal.IsZeroed(cfg.HardwareAddr[:]...) {
		return lneto.ErrInvalidConfig
	}
	seed := cfg.Seed
	if seed == 0 {
		// Derive a per-host seed from the MAC address.
		for _, b := range cfg.HardwareAddr {
			seed = seed<<8 | uint64(b)
		}
	}
	first := cfg.FirstCandidate
	haveFirst := ipv4.IsLinkLocal(first) && first[2] >= 1 && first[2] <= 254
	*h = Handler{
		connID:         h.connID + 1,
		now:            cfg.Now,
		prng:           uint32(seed) ^ uint32(seed>>32),
		hw:             cfg.HardwareAddr,
		firstCandidate: first,
		haveFirst:      haveFirst,
	}
	if h.prng == 0 {
		h.prng = 1 // xorshift cannot escape the zero state.
	}
