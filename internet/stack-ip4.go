...
func (sb *StackIP) Reset(addr netip.Addr, maxNodes int) error {
	...
	*sb = StackIP{
		...
		stackip4: stackip4{
			ip4:             sb.stackip4.ip4,
			ipID:            1,
			acceptMulticast: sb.stackip4.acceptMulticast,
			vld:             sb.stackip4.vld,
		},
	}
	if sb.stackip4.vld == nil {
		sb.stackip4.vld = new(lneto.Validator)
	}
	return nil
}
...
func (sb stackip4) prepEncaps(ipfrm []byte) uint16 {
	ifrm, _ := ipv4.NewFrame(ipfrm)
	const ihl = 5
	const headerlen = ihl * 4
	const dontFrag = 0x4000
	ifrm.SetVersionAndIHL(4, ihl)
...
func (sb *stackip4) finishEncaps(ipfrm []byte, proto lneto.IPProto, totalLen uint16) {
	if len(ipfrm) < int(totalLen) {
		panic("invalid totalLen")
	}
	ifrm, _ := ipv4.NewFrame(ipfrm)
...
func (sb *stackip4) encapsulate(carrierData []byte, mustBeZero, offsetToIpfram int) (int, error) {
	ifrm, _ := ipv4.NewFrame(carrierData[offsetToIpfram:])
	const ihl = 5
	const headerlen = ihl * 4
	const dontFrag = 0x4000
	ifrm.SetVersionAndIHL(4, ihl)
...
func (sb *stackip4) demux(ipfrm []byte) error {
	debugLog("ip:demux")
	ifrm, err := ipv4.NewFrame(ipfrm)
	if err != nil {
		return err
	}
	dst := ifrm.DestinationAddr()
	...
	sb.handlers.info("ipDemux", slog.String("ipproto", proto.String()), slog.Int("plen", int(totalLen)))
	err = node.callbacks.Demux(ipfrm[:totalLen], int(ihl*4))
	if sb.handlers.tryHandleError(node, err) {
		sb.handlers.info("ipclose", slog.String("proto", proto.String()))
		err = nil
	}
