...
func (h *Handler) Recv(buf []byte) error {
	...
	// TODO(RFC 768 / RFC 8200 §8.1 checksum): the UDP checksum is neither
	// verified here nor computed on transmit (Frame only exposes CRC get/set).
	// It is optional over IPv4 but mandatory over IPv6, where the pseudo-header
	// checksum must be computed and a zero result transmitted as 0xFFFF. Wire it
	// up (verify on Recv, compute on send) for full correctness.
	ul := ufrm.Length()
	if ul < sizeHeader {
