...
func (conn *Conn) Configure(config ConnConfig) (err error) {
	...
	if config.LossRecovery != nil && config.Nanotime == nil {
		// The tcp package holds no clock: a loss-recovery algorithm cannot run
		// without a time source to stamp its rx/tx hooks. This is the single
		// sanctioned error site; the transmit hot path stays error-free.
