...
| TCP selective acknowledgment | RFC 2018 | 🟡 | `tcp` | 0 | Opt-in via `EnableSACK`; SACK-permitted negotiation, receiver blocks from the reassembly buffer, sender selective retransmit |
| TCP timestamps | RFC 7323 | 🟡 | `tcp` | 0 | Opt-in via `EnableTimestamps`; option negotiation and per-ACK RTT measurement (RTTM). PAWS and Window Scaling not implemented |
| TCP out-of-order reassembly | RFC 9293 | 🟡 | `tcp` | 0 | Always-on bounded buffer once `SetBuffers` is called; single gap filled without go-back-N |
